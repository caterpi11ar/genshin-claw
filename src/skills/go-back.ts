...
export const goBack = defineSkill({
  ...
  async handler(input, ctx) {
    ...
    const response = await session.page.goBack({
      waitUntil: input.waitUntil ?? "load",
    });
    if (!response) {
      return { content: "No previous page in history", success: false };
    }
    const title = await session.page.title();
    return {
      content: `Navigated back to ${session.page.url()} — "${title}"`,
      success: true,
    };
  },
});
